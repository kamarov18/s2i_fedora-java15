...
ENV JOLOKIA_VERSION="1.7.0" \
    PROMETHEUS_JMX_EXPORTER_VERSION="0.16.0" \
...
LABEL io.fabric8.s2i.version.maven="3.8.3" \
      io.fabric8.s2i.version.jolokia="1.6.2" \
      io.fabric8.s2i.version.prometheus.jmx_exporter="0.16.0" \
...
# Prometheus JMX exporter agent
 RUN mkdir -p /opt/prometheus/etc \
  && curl https://repo1.maven.org/maven2/io/prometheus/jmx/jmx_prometheus_javaagent/0.16.0/jmx_prometheus_javaagent-0.16.0.jar \
...
RUN mkdir -p /opt/jolokia/etc \
 && curl https://repo1.maven.org/maven2/org/jolokia/jolokia-jvm/1.7.0/jolokia-jvm-1.7.0-agent.jar \
